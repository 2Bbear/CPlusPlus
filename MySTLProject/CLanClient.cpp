...
unsigned __stdcall ConnectThread(LPVOID arg)
{
    CLanClient* self = (CLanClient*)arg;
    if (self == nullptr)
    {
        return 0;
    }
...
#ifdef UNICODE
bool CLanClient::Start(const wchar_t* _ip, int _port, int _runingWorkerThreadCnt, int _waitWorkerThreadCnt, bool _isDoNagle, unsigned long long _maxConnectClient)
#else
bool CLanClient::Start(const char* _ip, int _port, int _runingWorkerThreadCnt, int _waitWorkerThreadCnt, bool _isDoNagle, unsigned long long _maxConnectClient)
#endif // UNICODE
{
    ...
    if (isAlreadyStart==false)
    {
        ...
        _LOGW(ELogLevel::Level_NOTICE, L"CLanClient :CLanClient internal variable setting completed  \n");
        //세션 공간 할당
        ...
        _LOGW(ELogLevel::Level_NOTICE, L"CLanClient :Successfully allocating memory space for session \n");
        ...
        _LOGW(ELogLevel::Level_NOTICE, L"CLanClient :Session index allocation complete \n");
        ...
        _LOGW(ELogLevel::Level_NOTICE,L"CLanClient : CLanClient Object Initialize Complete.\n");
    }

    //Create Connect Thread
    HANDLE hConnectThread;
    hConnectThread = (HANDLE)_beginthreadex(0, 0, ConnectThread, this, 0, nullptr);
    if (hConnectThread == NULL)
    {
        return false;
    }
    CloseHandle(hConnectThread);
    _LOGW(ELogLevel::Level_NOTICE, L"CLanClient : Create Connect Thread complete \n");

    /*
    connect가 될때까지 기다려야할까?. 그래야 쓸떼없이 워커들을 만들필요도 없을테고.
    그러면 이곳에서 lock을 하나 잡아야하는데...
    */
    ...
    if (isAlreadyStart==false)
    {
        ...
        _LOGW(ELogLevel::Level_NOTICE, L"CLanClient : Create Worker thread Complete.\n");
    }
    isAlreadyStart = true;
    _LOGW(ELogLevel::Level_NOTICE, L"CLanClient : Start proccess Complete.\n");
    return true;
}

void CLanClient::Stop(bool _isShutdown)
{
    //남아있는 세션 아이디 로그로 남기기
    for (int i = 0; i < maxConnectClient; ++i)
    {
...
int CLanClient::GetClientCount()
{
    return 0;
}

bool CLanClient::Disconnect(unsigned long long _sessionID)
{
    return false;
}


bool CLanClient::SendPacket(unsigned long long _sessionID, SerializableBuffer* _paload)
{
...
bool CLanClient::InputSendPacket(unsigned long long _sessionID, SerializableBuffer* _packet)
{
    int curThreadID = GetCurrentThreadId();
    CSession* session = FindSession(_sessionID);
    if (session == nullptr)
...
void CLanClient::WorkerThreadFunc()
{
    int curThreadID = GetCurrentThreadId();
...
void CLanClient::ConnectThreadFunc()
{
    //리슨 소켓 생성
    ...
    unsigned int currentRetryConnect = 0;
    //Connect 요청
    while (true)
    {
        if (connect(listen_sock, (struct sockaddr*)&serveraddr, sizeof(serveraddr)) != SOCKET_ERROR)
        {
            //접속에 관한 처리
            _LOGW(ELogLevel::Level_NOTICE, L"connect succeed!! \n");
            ...
            if (session == nullptr)
            {
                ...
                _LOGW(ELogLevel::Level_ERROR, L"Connect Error 세션 확보 실패 IP : %s, Port : %d curSessionCount : %d\n", ip, port, curSessionCount);
                continue;
            }
            //Setting Session...
            ...
            SuccessServerJoin(session->sessionID);
            //세션에서 부터 네트워크 통신 시작!===========
            //소켓과 IOCP 연결
            CreateIoCompletionPort((HANDLE)listen_sock, hCCP, (ULONG_PTR)session, 0); // 연결하는 순간 이 소켓은 넌 블락 소켓이 됨.
        ...
        else
        {
            ...
            _LOGW(ELogLevel::Level_ERROR, L"connect fail...ip: %s, port : %d\n", ip,port);
        }
        //재연결 최대 시도를 초과했을 경우 연결 포기.
        if (retryConnectionCount<=currentRetryConnect)
        {
            _LOGW(ELogLevel::Level_NOTICE, L"Server connection retry count exceeded : %d count\n", currentRetryConnect);
            closesocket(listen_sock);
            OnFailConnectServerJoin();
            return;
        }
        ++currentRetryConnect;
    }
...
void CLanClient::SendPost(CSession* session)
{
...
void CLanClient::RecvPost(CSession* _session)
{
    int curThreadID = GetCurrentThreadId();
    DWORD recvbytes;
    DWORD flag = 0;
...
void CLanClient::Release(unsigned long long _sessionId)
{
...
CSession* CLanClient::FindSession(unsigned long long _sessionID)
{
    for (int i = 0; i < maxConnectClient; ++i)
    {
        CSession* session = sessionArr + i;
...
void CLanClient::TestFunc(char* packetPtr, DWORD _size)
{
    unsigned long long idx = 0;
    while (idx < _size)
    {
